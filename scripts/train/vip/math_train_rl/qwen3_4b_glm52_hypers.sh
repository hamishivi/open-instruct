#!/bin/bash
# Value warmup + RL: answer_prefix GT conditioning with decoupled length-adaptive GAE
# (no SAE), group size 1, skip-tool-outputs GAE, and TIS ratio masking.
#
# Compared to `qwen3_4b_vpretrain100_rl1k_sae_answer_prefix.sh` (glm52-style hypers):
#   * group size 1 with prompts scaled 32 -> 256 (same overall bsz = 256)
...
#   * --value_num_epochs 2
#   * 100 value-only warmup steps, then 1000 RL steps
#
# Step budget:
#   total_episodes = (100 value warmup + 1000 RL) * 256 prompts * 1 sample
#                  = 281600
DDMM=$(date +"%d%m")
