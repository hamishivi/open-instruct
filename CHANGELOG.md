...
### Fixed
- Align scalar and generative critic training with the policy path, including correct loss scaling and token accounting, asynchronous DeepSpeed critic updates, health-checked weight synchronization, DPPO, bounded value predictions, fail-loud context validation, and consistent metrics/checkpoint handling (https://github.com/hamishivi/open-instruct/pull/1).
- Fix `AttributeError: 'list' object has no attribute 'shape'` in `UlyssesSPSplitter.split_collated_batch` when `sequence_parallel_size > 1` and value-model fields (`ground_truths`, `sibling_rollouts`, `hints`) are present: non-tensor fields are now passed through unchanged instead of being fed to `F.pad` (https://github.com/allenai/open-instruct/pull/TODO).
- Fix OOM in value loss backward when using `--value_model_ground_truth_conditioning` with long packs: `_forward_value_with_conditioning` now processes sub-sequences sequentially for the gradient (loss) pass (`sequential=True`) to bound peak activation memory during gradient-checkpoint recomputation, while keeping the batched single-call approach for the no-grad scoring pass to prevent cross-PG NCCL deadlocks (https://github.com/allenai/open-instruct/pull/TODO).
- Fix double-0.5 coefficient in `value_clipped_mse_loss`: both the clipped and unclipped MSE paths were incorrectly multiplied by 0.5 (a PPO2 legacy factor already folded into `value_loss_coef`), halving the effective value loss (https://github.com/allenai/open-instruct/pull/TODO).
- Fix value loss mask off-by-one: the mask used for the value loss was `response_masks[:, 1:]` (shifted left), incorrectly including the last prompt token and missing the first response token. Changed to `response_masks[:, :-1]` so V(s_t) is supervised at the correct response positions (https://github.com/allenai/open-instruct/pull/TODO).
- Replace per-sample advantage whitening with global advantage whitening via `dist.all_reduce` across all samples and ranks, matching standard PPO practice and eliminating sample-by-sample variance inflation (https://github.com/allenai/open-instruct/pull/TODO).
- Clear stale ZeRO-3 `ds_active_sub_modules` tracking before `GatheredParameters` in `_save_value_model` to prevent hang on save; wrap value model checkpoint save in try/except in `save_checkpoint_state` (https://github.com/allenai/open-instruct/pull/TODO).
- Add per-token value diagnostics logged every training step: explained variance, returns/predictions mean+std, and per-response-percentile mean values split by correct vs incorrect rollouts (https://github.com/allenai/open-instruct/pull/TODO).
- Fix cross-PG NCCL deadlock during value/policy warmup: weight sync was being triggered every step despite the skip-during-warmup guard being unreachable dead code (the `elif in_warmup` branch was shadowed by the preceding `if training_step > resume_training_step`). Swapping the branch order ensures weight sync is correctly skipped while the policy is frozen (https://github.com/allenai/open-instruct/pull/TODO).
- Fix cross-PG NCCL deadlock during GT-conditioned value model training: `_forward_value_with_conditioning` previously called the value model once per sub-sequence in a loop, producing N×L ZeRO-3 allgather cycles per step. Replaced with a single batched forward over all sub-sequences (pad to uniform length, one model call), collapsing to one allgather cycle regardless of pack density and eliminating the race window with the concurrent weight sync thread (https://github.com/allenai/open-instruct/pull/TODO).
- Wrap frozen-policy forward pass in `torch.no_grad()` when `skip_policy_update=True` to avoid unnecessary computation graph memory overhead (https://github.com/allenai/open-instruct/pull/TODO).
